...
SimpleFOC does synchronize (center-align) the PWM signals generated by different timers in some cases (as shown on the table above), but it is always better to use the pins that belong to the same timer.
</blockquote>

<blockquote class="info" markdown="1"> <p class="heading">BEWARE: Timer-ADC sync - low-side current sensing</p>
Timer and ADC synchronization, required for low-side current sensing is only available for certain families of microcontrollers: stm32, esp32 (`MCPWM` based) and Teesny 4.
</blockquote>
...
## Arduino AVR boards
For Arduino AVR boards (UNO, MEGA, Nano, Leonardo, ...), the PWM pins are usually marked with a `~` sign. These pins are the ones that can generate PWM signals. If you are writing an application with 2PWM,3PWM or 4PWM modes, you can use any of the PWM pins. The PWM timers are mutually synchronized so you do not have to worry about this info. 
...
Stm32 is the most powerful family of microcontrollers supported by SimpleFOC, at least in terms of motor control capabilities. SimpleFOC supports most of the STM32 boards and all of them can be used in 6PWM mode. Additionally from the release [v2.3.3](https://github.com/simplefoc/Arduino-FOC/releases/tag/v2.3.3) the SimpleFOC library synchronizes the PWM signals generated by different timers by default. Therefore for all the use cases except 6PWM generation, the user does not have to worry about the timers associated with the pins, however it is still recommended to use the pins that belong to the same timers. 
...
This is the recommended as all stm32 boards have timers typically `TIM1` and `TIM8` that are designed for this applications and allow for the best performance.
...
Finding the pins that belong to the same timer is not an easy task, especially for less experienced users. Therefore, we've created a website that enables a relatively easy navigation of the pins and timers for the most popular stm32 boards. 
...
SimpleFOC does not synchronize the PWM signals generated by different timers for Teensy boards, so it is recommended to use the pins that belong to the same timer.
